...
async function askOpenRouter(chatId, userMessage, modelKey = 'deepseek-v3') {
    try {
        ...
        if (error.status === 401) {
            return '❌ Ошибка авторизации OpenRouter. Проверь API ключ в .env файле.';
        }
        if (error.status === 429) {
            return '⏳ Слишком много запросов или закончились токены. Попробуй позже.';
        }
        if (error.code === 'ECONNREFUSED') {
            return '🌐 Нет соединения с OpenRouter. Проверь интернет.';
        }
        return '😵 Извини, у меня ошибка с ИИ. Попробуй еще раз позже.';
    }
}
